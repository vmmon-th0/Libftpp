...
struct IVector3
{
    ...
    constexpr IVector3() noexcept
        : x(0), y(0), z(0)
    {
    }

    constexpr IVector3(TType x, TType y, TType z) noexcept
        : x(x), y(y), z(z)
    {
    }
    ...
    {
        return
            static_cast<float>(x) * static_cast<float>(vec.x) +
    ...
    {
        return IVector3<float>(
            static_cast<float>(y) * static_cast<float>(vec.z) -
            static_cast<float>(z) * static_cast<float>(vec.y),

            static_cast<float>(z) * static_cast<float>(vec.x) -
            static_cast<float>(x) * static_cast<float>(vec.z),

            static_cast<float>(x) * static_cast<float>(vec.y) -
            static_cast<float>(y) * static_cast<float>(vec.x)
        );
    }
    ...
    template<typename U>
    constexpr IVector3<std::common_type_t<TType,U>>
    operator+(IVector3<U> const& o) const noexcept
    {
        using R = std::common_type_t<TType,U>;
        return IVector3<R>(
            static_cast<R>(x) + static_cast<R>(o.x),
            static_cast<R>(y) + static_cast<R>(o.y),
