...
// We overload the () operator to generate our PRNG in that way: By multiplying each coordinate by a distinct and
// "large" prime number, we change the numerical "frequency" of each coordinate in integer space. This prevents the same
// patterns (especially linear) in the coordinates from resulting in identical (collision) or similar values after the
// combination.
